                    ...
                    <form action='{{ route("admin.admin.update", [$user->id]) }}' method="POST"
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="text" class="form-control" placeholder="Name" aria-label="Name" required=""
                                value="{{ old('name', isset($user) ? $user->full_name : '') }}" name="full_name">
                        </div>
                        <div class="col-md-6">
                            <label for="inputEmail4" class="form-label">Email</label>
                            <input type="email" class="form-control" id="inputEmail4" placeholder="Email id" required=""
                                value="{{ old('email', isset($user) ? $user->email : '') }}" name="email">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Phone</label>
                            <input type="text" class="form-control" placeholder="Phone" required=""
                                value="{{ old('phone', isset($user) ? $user->phone_number : '') }}" name="phone_number">
                        </div>
